...
策定日: 2026-08-02
最終改訂: 2026-08-15（4.1 に `comment` の格納先を追加、4.4 を新設して ID3 で扱わない理由を記録）
前回改訂: 2026-08-11（3.5 に規則8「作品名の言語」を追加。非ラテン文字の原語と `No.` の書式も規則8 に含む）
前々回改訂: 2026-08-11（3.5 アルバム名の書式を確定し 6.1 を残作業に縮小、2.4 に `comment` を追加、6.9 を新設）
...
| `comment` | 自由記述の注記。版・稿の情報（ハース版／ノヴァーク版等）はここに置く。正規形を定めず、検査ルールの対象にしない。格納先は 4.1、ID3 で扱わない理由は 4.4 |
...
2. **`date` が一意に決まらない場合はアルバム名を書き換えず保留する。** 未設定の場合と、**単位内で値が割れている場合**の両方が対象。判定を保留する扱いは 7.5 と同じで、保留理由をコード（`HOLD_DATE_UNKNOWN`）で保持し、`date` が一意になった時点で自動的に再判定できるようにする。**推測で埋めない**（7章 原則4）
   ...
   - **主作品 + カップリング（規則5）では、年が割れていても単位の切り方は誤っていない**（2026-08-15 追記）。主作品と併録曲が別セッションであることは普通にある（ベートーヴェン 交響曲第6番 1971 + 序曲 2 曲 1972）。フォルダを分ければ 1 枚のアルバムが割れ（規則3 に反する）、年を揃えればタグが実際の録音年と食い違う。**この場合は主作品の録音年を採る。** 作曲家を主作品で代表させる規則5 と同じ扱いで、どれが主作品かは機械には決められないので、**個別例外（`albumOverrides`）に年を書く。** 書かれるまでは保留のままとする（推測で埋めない）
     - **選べるのは単位内にある年だけ**とする。どのファイルにも入っていない年を書けると、アルバム名だけが実在しない録音年を名乗る
     - **タグの年が一意なら、そちらを優先する。** 個別例外の年は割れているときだけ効く。一意な値まで上書きできると、タグを直したあとも辞書の古い指定が生き続け、食い違いが検出されない
...
6. **本物のコンピレーションには本規則を適用しない。** 主作品が定まらないアルバムは対象外とし、一覧に残して個別に決める（3.5 補足2 の (a)）
   - **判定は作曲家の数ではなく、主作品が定まるかどうかで行う**（2026-08-11 追記）。**作曲家が 1 人でも、複数の作品が 1 単位に入っていて主作品が定まらないものは対象外である。** 交響曲第4番と第5番を 1 枚に収めた盤、交響詩を 4 曲まとめた盤、歌劇の抜粋を集めた盤などが該当する
   - 規則5 との境目は主作品の有無だけである。「交響曲 1 曲 + 序曲 1 曲」は主作品が定まるので規則5、「交響曲 2 曲」は定まらないので規則6 になる
   - 2026-08-11 の実測では、複数の作曲家を含む 4 単位に加えて**単一の作曲家で 14 フォルダ**が該当した（3.5 補足3）
...
8. **作品名の言語は、ジャンル名を英語、固有の題名を原語とする**（2026-08-11 決定）
   ...
   - **ただし原語が非ラテン文字の場合は、英語圏での一般的な題名を用いる。ラテン転写は用いない。**
     3.1.2 規則2（団体名）と同じ扱いにする
     - ○ `The Nutcracker` / `The Sleeping Beauty` / `Swan Lake`
     - × `Shchelkunchik`（転写） ／ × `Щелкунчик`（キリル文字）
     - **理由は 3.1 と同じで、Android Auto の検索性である。** 検索はラテン文字の頭文字入力を前提としており、
       キリル文字（日本語と同様）は「その他大勢」に分類されて頭文字検索が働かない。作品名は `album` に入る値であり、
       アルバム一覧の検索対象そのものなので、人名・団体名と同じ制約を受ける
     - ラテン転写も採らない。転写は原語を読める人にも英語で覚えている人にも引けない値になり、
       検索性を回復しないまま表記だけが増える
   - **発音区別符号は 3.3 に従って正しく付ける。** 6.3（曲名中のウムラウト欠落）は既存の `title` をどう扱うかの話であり、作品エントリは人が新しく書くので迷いは生じない
   - `No.` の後には空白を入れる（`Symphony No. 8`）。現在のタグは `Symphony No.8` が大半だが、本書式では空白を入れた形を正とする
   - 3.1 の言語規則は人名・団体名に対するもので、作品名はその対象外である。作品名は本規則8 が唯一の根拠になる

##### 3.5 補足3：規則6（対象外）の実測（2026-08-11 / 検証済み）

作品エントリを埋める作業で全 220 単位を見た結果、**規則6 の対象は 18 フォルダ**だった。補足2 が挙げた「複数の作曲家を含む 4 単位」に加えて、**単一の作曲家で主作品が定まらないものが 14 フォルダある。**

| 種別 | フォルダ数 | 該当 |
|---|---|---|
| 複数の作曲家（補足2 の (a)） | 4 | `1812 スラブ行進曲`、`序曲集`、`トスカニーニ インター`、`シェエラザード` |
| 同一作曲家・複数作品 | 6 | `ニールセン 5,6`（交響曲4番+5番）、`ショスタコーヴィチ 祝典序曲・森の歌`（4 作品）、`リスト 交響詩`（4 曲）、`ビゼー カルメン・アルルの女`（組曲 4 つ）、`エグモント序曲・大フーガ`、`モツァ ピアノ協奏曲 21 23` |
| 抜粋・名曲集 | 6 | `ニーベルングの指環`、`ワーグナー - ショルティ - 指輪 ハイライト`、`ワーグナー - クナッパーツブッシュ`、`ワーグナー - ムラヴィンスキー`、`Wagner Opera Choruses`、`Wagner's Greatest Hits` |
| 作曲家フォルダ直下の寄せ集め | 2 | `グリーグ`（ピアノ協奏曲 + ペール・ギュント組曲 1・2）、`バッハ`（13 曲が全部別作品） |

**フォルダ名が中身と食い違っているものがある。** `ニールセン 5,6` の中身は交響曲第4番と第5番であり、`album` の `Nielsen Symphony No.4` も片方しか指していない。6.7（ファイル名・フォルダ名の未整理）の一種。

**`ニーベルングの指環` は全曲盤ではなく抜粋盤である。** `ワーグナー - ショルティ - 指輪 ハイライト` と収録曲が 6 曲重なっており（形式は m4a と FLAC で違う）、同一演奏の別所蔵である可能性がある。ファイルサイズが桁違いなので補足2 の D 種別（同一演奏の別リリース）とは断定できない。**未確認。**
...
### 4.4 comment の注意点（2026-08-15 決定）

**ID3v2（MP3 / AIFF）では `comment` を扱わない。** 読みも書きもしない。

ID3v2 の `COMM` フレームは description を持ち、iTunes がこれを使って `iTunNORM`（音量正規化）・
`iTunSMPB`・`iTunes_CDDB_IDs` を格納する。`COMM` を論理フィールドとして素直に扱うと、

- **読み**: description を区別せずに拾うため、コメント欄にバイナリ相当の文字列が出る
- **書き**: 値を空にしたときのフレーム削除が、`iTunNORM` ごと消す

実測（2026-08-15、対象ライブラリ）では次のとおりだった。

| 形式 | 件数 | 利用者のコメント（description が空の `COMM`） |
|---|---|---|
| AIFF | 11 | **0**（全件が description 付きの `COMM` を 3 個ずつ持つ） |
| MP3 | 4 | **0**（`COMM` 自体が無い） |

空 description のフレームだけを読み書きする特別扱いで回避はできるが、**得られるのは
15 ファイルで注記を編集できることだけで、その 15 ファイルに注記は 1 件も無い。**
`iTunNORM` の消失は不可逆（バックアップの `RawTags` は記録用で復元に使わない）なので、
釣り合わないと判断した。

M4A の `©cmt` と FLAC の `COMMENT` は通常のフィールドとして扱う。
FLAC で他ソフトが使う `DESCRIPTION` は対象外とする。

扱えない組み合わせを編集した場合、アプリは手編集の「気づき」で理由を知らせる。
編集そのものは止めない。
...
**実体IDの先頭には ISO 3166-1（alpha-2）の国コードを小文字で付けることを推奨する。** 本拠地の国を表す
接頭辞（`ru-` `uk-` `de-` `us-` 等）＋ハイフン＋団体名を略した短い識別子、という形にする。国コードを
先頭に置くと、同名・類似名の団体が別の国に存在する場合でも一覧上で見分けやすくなる。ダイアログの自動
提案（正規形をスラッグ化しただけの値）はこの形になっていないため、登録時に手で直すこと。
