...
策定日: 2026-08-02
最終改訂: 2026-08-11（3.5 アルバム名の書式を確定し 6.1 を残作業に縮小、2.4 に `comment` を追加、6.9 を新設）
前回改訂: 2026-08-03（2.3 に配役情報の実値を追記、6.8 曲名中のセミコロンを追加）
...
| `{作品名}` | **楽章名ではなく作品そのものの名前。** `title` からは取れない（`title` は楽章名）。正規化辞書の作品エントリを唯一の供給元とする |
| `{date}` | 録音年 4 桁（2.4） |
...
2. **`date` が決まらない場合はアルバム名を書き換えず保留する。** 判定を保留する扱いは 7.5 と同じで、保留理由をコード（`HOLD_DATE_UNKNOWN`）で保持し、`date` が埋まった時点で自動的に再判定できるようにする。**推測で埋めない**（7章 原則4）
...
**作品名の供給元が無い。** 3.5 の `{作品名}` は作品そのものの名前であり、`title`（楽章名）からも `album`（`Symphony No.5` のような汎用名）からも一意には取れない。正規化辞書に作品エントリを設けて供給元とする方針だが未整備。作品エントリが無いあいだ、そのアルバムは修正案を出せない。
...
**手掛かりはファイル名と `title` に出てくる作曲家名しかない。** これを照合するのが R-210 だが、**一致しても誤りとは限らない。** 曲名が別の作曲家の名前を正当に含む作品があるため。
...
2026-08-11 時点の検出は 3 ファイル / 1,078 ファイル、いずれも真の誤りだった。ただしこれは現在の辞書に対する値であり、ライブラリに他の誤りが無いことの根拠にはならない。
