...
`docs/SPEC.md` 6.1 の 25 ルールをすべて実装している。
...
### アルバム名のルール（R-501 / R-502）

書式は `{作曲家}: {作品名} - {date}/{artist}` で確定している（`TAGGING_POLICY.md` 3.5）。**修正案を出せないのは書式が未決だからではなく、`{作品名}` の唯一の供給元である辞書の作品エントリが未整備だからである**（`SPEC.md` 13章 D6）。根拠テキストの要素順（年が演奏者より先）は 3.5 に合わせること。
