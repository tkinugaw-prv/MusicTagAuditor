...
| このアルバムの扱いを決める | R-504 の `HOLD_WORK_UNKNOWN` と R-501 の明細。**`HOLD_DATE_UNKNOWN` / `HOLD_ARTIST_UNKNOWN` では押せない** | フォルダ、`disc` |
...
- 同じダイアログで「対象外にする」と「作曲家・作品名を指定する」を選べる。版の違い（規則4）・同一演奏の別リリース（規則7）は対象外ではなく上書きで扱う
  - **だからボタン名を「対象外にする」にしない**（2026-08-15 改称）。二択の片方だけを名乗ると、規則5（作曲家の指定）の入口がここだと気付けない
- **`HOLD_DATE_UNKNOWN` / `HOLD_ARTIST_UNKNOWN` ではボタンを出さない。** `albumOverrides` に `date` も `artist` も無いので解けない。それでも対象外にすれば一覧からは消えるので、押せるままだとタグが割れた単位を検出から消せてしまう。保留の根拠にファイル一覧タブでの直し方を書いてあるのも同じ理由（`SPEC.md` 7.4.4）
