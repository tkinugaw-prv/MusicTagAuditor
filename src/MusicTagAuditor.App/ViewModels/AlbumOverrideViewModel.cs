    ...
    private void UpdateNotice()
    {
        ...
        if (Excludes)
        {
            Notice = "本物のコンピレーション（TAGGING_POLICY 3.5 規則6）に使います。"
                + " 登録して再検査すると、この単位は R-504 と R-501 の一覧から消えます。";
        ...
        //
        // **R-501 から消えるのは作曲家を指定したときだけ**であることまで書く。R-501 が問うて
        // いるのは「主作品が定まるか」であり、作品名・年の指定はその答えになっていない。
        // 書いたのに消えない理由が画面から分からないと、同じ単位に何度も向き合うことになる。
        Notice = CanExclude
            ? "版の違い（規則4）・同一演奏の別リリース（規則7）は作品名の上書きで、"
                + " 主作品 + カップリング（規則5）は作曲家の指定で扱います。"
                + " 作曲家を指定すると、この単位は R-501 の一覧からも消えます"
                + "（作品名・年だけの指定では消えません）。"
